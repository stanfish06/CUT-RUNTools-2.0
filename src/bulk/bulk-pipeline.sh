...
elif [ "$adaptor_type" == "Nebnext" ]
then
    >&2 echo "[info] Use Nebnext adaptor  "
    $javabin/java -jar $trimmomaticbin/$trimmomaticjarfile PE -threads $cores -phred33 $dirname/"$base"_R1_001.fastq.gz $dirname/"$base"_R2_001.fastq.gz $trimdir/"$base"_1.paired.fastq.gz $trimdir/"$base"_1.unpaired.fastq.gz $trimdir/"$base"_2.paired.fastq.gz $trimdir/"$base"_2.unpaired.fastq.gz ILLUMINACLIP:$adapterpath/NEBNextAdaptorsPE.fa:2:15:4:4:true LEADING:20 TRAILING:20 SLIDINGWINDOW:4:15 MINLEN:25 > $logdir/${base}_trim1.log 2>&1
else
