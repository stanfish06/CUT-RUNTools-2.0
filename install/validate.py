...
if __name__ == "__main__":
        ...
        for x in flist:
            ...
            if m1 is None and m2 is None:
                print(
                    "Error: "
                    + x
                    + " no pattern of _R1_001.fastq.gz, or _R2_001.fastq.gz detected"
                )
                sys.exit(1)
            if m1 is not None:
    ...
    if not check_program_exists(config["software_config"]["samtoolsbin"], "samtools"):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["trimmomaticbin"],
        config["software_config"]["trimmomaticjarfile"],
    ):
        sys.exit(1)
    if not check_program_exists(config["software_config"]["macs2bin"], "macs2"):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["picardbin"],
        config["software_config"]["picardjarfile"],
    ):
        sys.exit(1)
    ...
    if not check_program_exists(config["software_config"]["pythonbin"], "python"):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["adapterpath"], "Truseq3.PE.fa"
    ):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["extratoolsbin"], "SEACR_1.1.sh"
    ):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["extratoolsbin"], "SEACR_1.1.R"
    ):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["extratoolsbin"], "change.bdg.py"
    ):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["extratoolsbin"], "get_summits_seacr.py"
    ):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["extratoolsbin"], "get_summits_broadPeak.py"
    ):
        sys.exit(1)
    if not check_program_exists(config["software_config"]["path_parallel"], "parallel"):
        sys.exit(1)
    if not check_program_exists(
        config["software_config"]["path_deeptools"], "deeptools"
    ):
        sys.exit(1)
    ...
    if not args["ignore_input_output"]:
        if not check_program_exists(
            os.path.dirname(config["software_config"]["genome_sequence"]),
            "%s.chrom.sizes" % config["input_output"]["genome"],
        ):
            sys.exit(1)
    ...
    if args["software"]:
        ...
        os.system(
            "%s/java -jar %s/%s -version"
            % (
                config["software_config"]["javabin"],
                config["software_config"]["trimmomaticbin"],
                config["software_config"]["trimmomaticjarfile"],
            )
        )
        ...
        cmd3 = (
            'ldlibrary=`echo $LD_LIBRARY_PATH | tr ":" "\\n" | grep -v %s | paste -s -d:` && unset LD_LIBRARY_PATH && export LD_LIBRARY_PATH=%s:$ldlibrary'
            % (p_pythonlib, p_pythonlib)
        )
        cmd4 = (
            'pythonlib=`echo $PYTHONPATH | tr ":" "\\n" | grep -v %s | paste -s -d:` && unset PYTHONPATH && export PYTHONPATH=%s:$pythonlib'
            % (
                config["software_config"]["macs2pythonlib"],
                config["software_config"]["macs2pythonlib"],
            )
        )
        ...
        cmd5 = (
            "%s/make_cut_matrix --version"
            % config["software_config"]["makecutmatrixbin"]
        )
